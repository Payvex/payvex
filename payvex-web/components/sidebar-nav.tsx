...
const menuItems = [
  ...
  },
  {
    ...
    activePrefix: "/docs",
    children: [
      { label: "Integrações e Plugins", href: "/docs/integrations" },
      { label: "Developers SDK", href: "/docs/developers" },
    ],
  },
];
    ...
    <aside
      className={cn(
        ...
        isCollapsed
          ...
          : "w-64 px-4 bg-surface border-r border-white/10 shadow-2xl",
      )}
    >
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className={cn(
          "absolute top-10 bg-primary text-surface rounded-full p-1.5 shadow-lg border-2 border-surface hover:scale-110 transition-all z-[60]",
                ...
                >
                  <div
                    className={cn(
                      ...
                      isCollapsed &&
                        (isActive
                          ? "bg-primary text-surface scale-110 shadow-[0_0_15px_rgba(141,255,0,0.4)]"
                          : "bg-surface text-white/50 hover:bg-[#4a528a] hover:text-white"),
                    )}
                  >
                    <item.icon
                    ...
                    <div className="flex items-center justify-between flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isRestrictedForUser && (
                          <Lock
                            size={12}
                            className="text-white/20 group-hover:text-primary/50 transition-colors"
                          />
                        )}
                        {item.children && (
                          <ChevronDown
              ...
              {item.children && !isCollapsed && isActive && (
                <div className="ml-8 space-y-1 border-l border-white/10 pl-3">
                  {item.children.map((child) => {
                    const isChildActive =
                      pathname === child.href ||
                      (pathname === "/docs" && child.href === "/docs/integrations");
      ...
      <div className="mt-auto mb-4">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <div
              className={cn(
                ...
                isCollapsed
                  ? "justify-center h-14 w-14 mx-auto rounded-2xl bg-surface border border-white/10 shadow-2xl hover:scale-105"
            ...
            >
              <div className="relative">
                <div
                  className={cn(
                    "rounded-full border-2 border-primary flex items-center justify-center text-xs font-bold text-white uppercase",
                    isCollapsed ? "h-9 w-9" : "h-10 w-10",
                  )}
                >
                  {userData ? getInitials(userData.name) : "PX"}
                </div>
                <div className="absolute -bottom-0.5 -right-0.5 h-3 w-3 bg-primary border-2 border-surface rounded-full" />
              </div>
          ...
          <DropdownMenuContent
            className="w-56 bg-surface border-white/10 text-white rounded-[0.625rem] shadow-2xl"
          ...
          >
            ...
            <DropdownMenuItem
              className="focus:bg-white/10 focus:text-primary cursor-pointer"
